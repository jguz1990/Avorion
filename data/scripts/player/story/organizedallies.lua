...
function add(factionIndex, amount)
    ...
    amount = amount or random():getInt(1, 2)
...
function onAssist(entityId)
    ...
    local entity = Entity(entityId)
    local relations = Player():getRelations(entity.factionIndex)
...
function makeDialog()
    ...
    d5_ThisIsNoneOfOur.answers = {
        {answer = "This is your chance to be the saviours of the galaxy."%_t, onSelect = "saveGalaxy", followUp = Dialog.empty()},
        ...
        {answer = "It's only a matter of time until the Xsotan will arrive here, too."%_t, onSelect = "arriveHereToo", followUp = Dialog.empty()},
...
function traitCheck(...)
    ...
    local threshold = lerp(Player():getRelations(faction.index), 25000, 70000, 0.95, 0.25)
