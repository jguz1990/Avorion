...
function initializeAIFaction(faction)
    ...
    local armed1, armed2, unarmed1
    while not armed1 or armed1.coaxial do
        armed1 = TurretGenerator.generateArmed(x, y, 0, Rarity(RarityType.Common))
    end

    while not armed2 or armed2.coaxial do
        armed2 = TurretGenerator.generateArmed(x, y, 0, Rarity(RarityType.Common))
    end

    while not unarmed1 or unarmed1.coaxial do
        unarmed1 = TurretGenerator.generate(x, y, 0, Rarity(RarityType.Common), WeaponType.MiningLaser)
    end
...
function initializePlayer(player)
    ...
    elseif server.difficulty == Difficulty.Easy then
        ...
        player:receive(2500000, 200000, 100000)
    end

    -- create turret generator
    local dps, tech = Balancing_GetSectorWeaponDPS(450, 0)
    local turret = InventoryTurret(TurretGenerator.generateSeeded(random:createSeed(), WeaponType.ChainGun, dps, tech, Rarity(RarityType.Uncommon), Material(MaterialType.Iron)))
    player:getInventory():add(turret, false)
    player:getInventory():add(turret, false)

    local dps, tech = Balancing_GetSectorMiningDPS(450, 0)
    local turret = InventoryTurret(TurretGenerator.generateSeeded(random:createSeed(), WeaponType.MiningLaser, dps, tech, Rarity(RarityType.Uncommon), Material(MaterialType.Iron)))
    player:getInventory():add(turret, false)
    player:getInventory():add(turret, false)
