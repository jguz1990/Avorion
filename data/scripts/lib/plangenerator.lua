...
function PlanGenerator.transformToCarrier(plan)
    ...
    local checkFree = function(block, offset)
        ...
        if box.size.y < 1.3 or box.size.z < 1.3 then
            return false
        end
...
function PlanGenerator.transformToFreighter(plan, faction)
    ...
    if stats.cargoHold < 100 then
        local scale = (100 / stats.cargoHold) ^ (1.0 / 3.0)
        plan:scale(vec3(scale, scale, scale))
    end
...
function PlanGenerator.selectShipStyle(faction, styleName)
