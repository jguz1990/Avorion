...
function Shop:initUI(interactionCaption, windowCaption, tabCaption)
    ...
    if not self.shared.window then
        local size = vec2(900, 900)
        local res = getResolution()

        local window = menu:createWindow(Rect(res * 0.70 - size * 0.5, res * 0.70 + size * 0.5));
    ...
    -- create buy tab
    self.buyTab = self.tabbedWindow:createTab("Buy"%_t, "data/textures/icons/bag.png", tabCaption)
...
--    window:createFrame(Rect(size))
    ...
    -- header
    window:createLabel(vec2(nameX, 0), "Name"%_t, 12)
    window:createLabel(vec2(materialX, 0), "Mat"%_t, 12)
    window:createLabel(vec2(priceX, 0), "Cr"%_t, 12)
    window:createLabel(vec2(stockX, 0), "#"%_t, 12)
