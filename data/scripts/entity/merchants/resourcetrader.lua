...
function ResourceDepot.initialize()
    ...
    if onServer() then
        ...
        -- resource shortage
        shortageTimer = -random():getInt(15 * 60, 30 * 60)
...
function ResourceDepot.getInitialResources()
    ...
    for i = 1, NumMaterials() do
        amounts[i] = math.max(0, probabilities[i - 1] - 0.1) * (getInt(500000, 1000000) * Balancing_GetSectorRichnessFactor(x, y))
    end
...
function ResourceDepot.initUI()
    ...
    menu:registerWindow(window, "Trade Resources"%_t);
...
function ResourceDepot.startShortage()
    ...
    shortageAmount = random():getInt(10, 50) * 10000
    ...
    local text = "We need ${amount} ${material}, quickly! If you can deliver in the next 30 minutes we will pay you handsomely."%_T
